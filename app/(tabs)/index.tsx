...
export default function HomeScreen() {
    ...
    try {
      ...
      if (selectedMode === 'chat') {
        ...
        const script = await generateText(
          `Write a short, engaging audio script about: ${query}. Keep it under 300 words.`
        );
  ...
  const ModeChip = ({ mode }: { mode: (typeof MODES)[number] }) => {
    ...
    return (
      ...
      >
        ...
        <Text
          style={[
            styles.modeLabel,
            active && { color: mode.color, fontWeight: '600' },
          ]}
        >
          {mode.label}
        </Text>
          ...
          <View style={styles.featureGrid}>
            {MODES.filter((m) => m.id !== 'more').map((mode) => (
              ...
              >
                ...
                <Text style={[styles.featureLabel, { color: mode.color }]}>
                  {mode.label}
                </Text>
